...
function init(botLogoPath) {
    ...
    template = ` <button class='chat-btn'><img src = "chatbot/icons/comment.png" class = "material-icon" ></button>
    ...
    <div class='chat-popup'>
		...
		<div class='chat-header'>
			...
			<button class = "expand-chat-window" ><img src="chatbot/icons/open_fullscreen.png" class="material-icon" ></button>
		</div>
		...
		<div class='chat-input-area'>
			<input type='text' id="chat_input" autofocus class='chat-input' onkeypress='return givenUserInput(event)' placeholder='Type a message ...' autocomplete='off'>
    ...
    chatBtn.addEventListener("click", () => {
        ...
        if (chatPopup.style.display == "none" && mobileDevice) {
            ...
            chatPopup.style.zIndex = 99;
            chatInput.focus();
            chatBtn.innerHTML = `<img src = "chatbot/icons/close.png" class = "material-icon" >`
        } else if (mobileDevice) {
            chatPopup.style.display = "none"
            chatBtn.innerHTML = `<img src = "chatbot/icons/comment.png" class = "material-icon" >`
        } else {
            mobileView()
        }
    })
    ...
    expandWindow.addEventListener("click", (e) => {
        ...
        if (expandWindow.innerHTML == '<img src="chatbot/icons/open_fullscreen.png" class="material-icon">') {
            expandWindow.innerHTML = `<img src = "chatbot/icons/close_fullscreen.png" class = 'material-icon'>`
            root.style.setProperty('--chat-window-height', 80 + "%");
            root.style.setProperty('--chat-window-total-width', 85 + "%");
            chatHeader.style.width = "100%";
        } else if (expandWindow.innerHTML == '<img src="chatbot/icons/close.png" class="material-icon">') {
            chatPopup.style.display = "none"
            chatBtn.style.display = "block"
        } else {
            expandWindow.innerHTML = `<img src = "chatbot/icons/open_fullscreen.png" class = "material-icon" >`
...
function mobileView() {
    ...
    if (chatPopup.style.display == "none") {
        ...
        expandWindow.innerHTML = `<img src = "chatbot/icons/close.png" class = "material-icon" >`
    }
}
